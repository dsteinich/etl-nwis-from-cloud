package gov.acwi.wqp.etl.nwqDataChecks;

import java.time.LocalDate;

public class NawqaSites {

    private String nwisHostNm;
    private String dbNo;
    private String agencyCd;
    private String siteNo;
    private String stationNm;
    private String stationIx;
    private String latVa;
    private String longVa;
    private String decLatVa;
    private String decLongVa;
    private String coordMethCd;
    private String coordAcyCd;
    private String coordDatumCd;
    private String districtCd;
    private String landNetDs;
    private String mapNm;
    private String countryCd;
    private String stateCd;
    private String countyCd;
    private String mcdCd;
    private String mapScaleFc;
    private String altVa;
    private String altMethCd;
    private String altAcyVa;
    private String altDatumCd;
    private String hucCd;
    private String agencyUseCd;
    private String basinCd;
    private String siteTpCd;
    private String topoCd;
    private String dataTypesCd;
    private String instrumentsCd;
    private String siteRmksTx;
    private String inventoryDt;
    private String drainAreaVa;
    private String contribDrainAreaVa;
    private String tzCd;
    private boolean localTimeFg;
    private String gwFileCd;
    private String constructionDt;
    private String reliabilityCd;
    private String aqfrCd;
    private String natAqfrCd;
    private String siteUse1Cd;
    private String siteUse2Cd;
    private String siteUse3Cd;
    private String waterUse1Cd;
    private String waterUse2Cd;
    private String waterUse3Cd;
    private String natWaterUseCd;
    private String aqfrTypeCd;
    private String wellDepthVa;
    private String holeDepthVa;
    private String depthSrcCd;
    private String projectNo;
    private String siteWebCd;
    private String siteCn;
    private LocalDate siteCr;
    private String siteMn;
    private LocalDate siteMd;
    private boolean deprecatedFg;
    private String siteLd;
    private boolean nawqaSiteFg;
    private boolean nasqanNmnSiteFg;

    public String getNwisHostNm() {
        return nwisHostNm;
    }

    public void setNwisHostNm(String nwisHostNm) {
        this.nwisHostNm = nwisHostNm;
    }

    public String getDbNo() {
        return dbNo;
    }

    public void setDbNo(String dbNo) {
        this.dbNo = dbNo;
    }

    public String getAgencyCd() {
        return agencyCd;
    }

    public void setAgencyCd(String agencyCd) {
        this.agencyCd = agencyCd;
    }

    public String getSiteNo() {
        return siteNo;
    }

    public void setSiteNo(String siteNo) {
        this.siteNo = siteNo;
    }

    public String getStationNm() {
        return stationNm;
    }

    public void setStationNm(String stationNm) {
        this.stationNm = stationNm;
    }

    public String getStationIx() {
        return stationIx;
    }

    public void setStationIx(String stationIx) {
        this.stationIx = stationIx;
    }

    public String getLatVa() {
        return latVa;
    }

    public void setLatVa(String latVa) {
        this.latVa = latVa;
    }

    public String getLongVa() {
        return longVa;
    }

    public void setLongVa(String longVa) {
        this.longVa = longVa;
    }

    public String getDecLatVa() {
        return decLatVa;
    }

    public void setDecLatVa(String decLatVa) {
        this.decLatVa = decLatVa;
    }

    public String getDecLongVa() {
        return decLongVa;
    }

    public void setDecLongVa(String decLongVa) {
        this.decLongVa = decLongVa;
    }

    public String getCoordMethCd() {
        return coordMethCd;
    }

    public void setCoordMethCd(String coordMethCd) {
        this.coordMethCd = coordMethCd;
    }

    public String getCoordAcyCd() {
        return coordAcyCd;
    }

    public void setCoordAcyCd(String coordAcyCd) {
        this.coordAcyCd = coordAcyCd;
    }

    public String getCoordDatumCd() {
        return coordDatumCd;
    }

    public void setCoordDatumCd(String coordDatumCd) {
        this.coordDatumCd = coordDatumCd;
    }

    public String getDistrictCd() {
        return districtCd;
    }

    public void setDistrictCd(String districtCd) {
        this.districtCd = districtCd;
    }

    public String getLandNetDs() {
        return landNetDs;
    }

    public void setLandNetDs(String landNetDs) {
        this.landNetDs = landNetDs;
    }

    public String getMapNm() {
        return mapNm;
    }

    public void setMapNm(String mapNm) {
        this.mapNm = mapNm;
    }

    public String getCountryCd() {
        return countryCd;
    }

    public void setCountryCd(String countryCd) {
        this.countryCd = countryCd;
    }

    public String getStateCd() {
        return stateCd;
    }

    public void setStateCd(String stateCd) {
        this.stateCd = stateCd;
    }

    public String getCountyCd() {
        return countyCd;
    }

    public void setCountyCd(String countyCd) {
        this.countyCd = countyCd;
    }

    public String getMcdCd() {
        return mcdCd;
    }

    public void setMcdCd(String mcdCd) {
        this.mcdCd = mcdCd;
    }

    public String getMapScaleFc() {
        return mapScaleFc;
    }

    public void setMapScaleFc(String mapScaleFc) {
        this.mapScaleFc = mapScaleFc;
    }

    public String getAltVa() {
        return altVa;
    }

    public void setAltVa(String altVa) {
        this.altVa = altVa;
    }

    public String getAltMethCd() {
        return altMethCd;
    }

    public void setAltMethCd(String altMethCd) {
        this.altMethCd = altMethCd;
    }

    public String getAltAcyVa() {
        return altAcyVa;
    }

    public void setAltAcyVa(String altAcyVa) {
        this.altAcyVa = altAcyVa;
    }

    public String getAltDatumCd() {
        return altDatumCd;
    }

    public void setAltDatumCd(String altDatumCd) {
        this.altDatumCd = altDatumCd;
    }

    public String getHucCd() {
        return hucCd;
    }

    public void setHucCd(String hucCd) {
        this.hucCd = hucCd;
    }

    public String getAgencyUseCd() {
        return agencyUseCd;
    }

    public void setAgencyUseCd(String agencyUseCd) {
        this.agencyUseCd = agencyUseCd;
    }

    public String getBasinCd() {
        return basinCd;
    }

    public void setBasinCd(String basinCd) {
        this.basinCd = basinCd;
    }

    public String getSiteTpCd() {
        return siteTpCd;
    }

    public void setSiteTpCd(String siteTpCd) {
        this.siteTpCd = siteTpCd;
    }

    public String getTopoCd() {
        return topoCd;
    }

    public void setTopoCd(String topoCd) {
        this.topoCd = topoCd;
    }

    public String getDataTypesCd() {
        return dataTypesCd;
    }

    public void setDataTypesCd(String dataTypesCd) {
        this.dataTypesCd = dataTypesCd;
    }

    public String getInstrumentsCd() {
        return instrumentsCd;
    }

    public void setInstrumentsCd(String instrumentsCd) {
        this.instrumentsCd = instrumentsCd;
    }

    public String getSiteRmksTx() {
        return siteRmksTx;
    }

    public void setSiteRmksTx(String siteRmksTx) {
        this.siteRmksTx = siteRmksTx;
    }

    public String getInventoryDt() {
        return inventoryDt;
    }

    public void setInventoryDt(String inventoryDt) {
        this.inventoryDt = inventoryDt;
    }

    public String getDrainAreaVa() {
        return drainAreaVa;
    }

    public void setDrainAreaVa(String drainAreaVa) {
        this.drainAreaVa = drainAreaVa;
    }

    public String getContribDrainAreaVa() {
        return contribDrainAreaVa;
    }

    public void setContribDrainAreaVa(String contribDrainAreaVa) {
        this.contribDrainAreaVa = contribDrainAreaVa;
    }

    public String getTzCd() {
        return tzCd;
    }

    public void setTzCd(String tzCd) {
        this.tzCd = tzCd;
    }

    public boolean isLocalTimeFg() {
        return localTimeFg;
    }

    public void setLocalTimeFg(boolean localTimeFg) {
        this.localTimeFg = localTimeFg;
    }

    public String getGwFileCd() {
        return gwFileCd;
    }

    public void setGwFileCd(String gwFileCd) {
        this.gwFileCd = gwFileCd;
    }

    public String getConstructionDt() {
        return constructionDt;
    }

    public void setConstructionDt(String constructionDt) {
        this.constructionDt = constructionDt;
    }

    public String getReliabilityCd() {
        return reliabilityCd;
    }

    public void setReliabilityCd(String reliabilityCd) {
        this.reliabilityCd = reliabilityCd;
    }

    public String getAqfrCd() {
        return aqfrCd;
    }

    public void setAqfrCd(String aqfrCd) {
        this.aqfrCd = aqfrCd;
    }

    public String getNatAqfrCd() {
        return natAqfrCd;
    }

    public void setNatAqfrCd(String natAqfrCd) {
        this.natAqfrCd = natAqfrCd;
    }

    public String getSiteUse1Cd() {
        return siteUse1Cd;
    }

    public void setSiteUse1Cd(String siteUse1Cd) {
        this.siteUse1Cd = siteUse1Cd;
    }

    public String getSiteUse2Cd() {
        return siteUse2Cd;
    }

    public void setSiteUse2Cd(String siteUse2Cd) {
        this.siteUse2Cd = siteUse2Cd;
    }

    public String getSiteUse3Cd() {
        return siteUse3Cd;
    }

    public void setSiteUse3Cd(String siteUse3Cd) {
        this.siteUse3Cd = siteUse3Cd;
    }

    public String getWaterUse1Cd() {
        return waterUse1Cd;
    }

    public void setWaterUse1Cd(String waterUse1Cd) {
        this.waterUse1Cd = waterUse1Cd;
    }

    public String getWaterUse2Cd() {
        return waterUse2Cd;
    }

    public void setWaterUse2Cd(String waterUse2Cd) {
        this.waterUse2Cd = waterUse2Cd;
    }

    public String getWaterUse3Cd() {
        return waterUse3Cd;
    }

    public void setWaterUse3Cd(String waterUse3Cd) {
        this.waterUse3Cd = waterUse3Cd;
    }

    public String getNatWaterUseCd() {
        return natWaterUseCd;
    }

    public void setNatWaterUseCd(String natWaterUseCd) {
        this.natWaterUseCd = natWaterUseCd;
    }

    public String getAqfrTypeCd() {
        return aqfrTypeCd;
    }

    public void setAqfrTypeCd(String aqfrTypeCd) {
        this.aqfrTypeCd = aqfrTypeCd;
    }

    public String getWellDepthVa() {
        return wellDepthVa;
    }

    public void setWellDepthVa(String wellDepthVa) {
        this.wellDepthVa = wellDepthVa;
    }

    public String getHoleDepthVa() {
        return holeDepthVa;
    }

    public void setHoleDepthVa(String holeDepthVa) {
        this.holeDepthVa = holeDepthVa;
    }

    public String getDepthSrcCd() {
        return depthSrcCd;
    }

    public void setDepthSrcCd(String depthSrcCd) {
        this.depthSrcCd = depthSrcCd;
    }

    public String getProjectNo() {
        return projectNo;
    }

    public void setProjectNo(String projectNo) {
        this.projectNo = projectNo;
    }

    public String getSiteWebCd() {
        return siteWebCd;
    }

    public void setSiteWebCd(String siteWebCd) {
        this.siteWebCd = siteWebCd;
    }

    public String getSiteCn() {
        return siteCn;
    }

    public void setSiteCn(String siteCn) {
        this.siteCn = siteCn;
    }

    public LocalDate getSiteCr() {
        return siteCr;
    }

    public void setSiteCr(LocalDate siteCr) {
        this.siteCr = siteCr;
    }

    public String getSiteMn() {
        return siteMn;
    }

    public void setSiteMn(String siteMn) {
        this.siteMn = siteMn;
    }

    public LocalDate getSiteMd() {
        return siteMd;
    }

    public void setSiteMd(LocalDate siteMd) {
        this.siteMd = siteMd;
    }

    public boolean isDeprecatedFg() {
        return deprecatedFg;
    }

    public void setDeprecatedFg(boolean deprecatedFg) {
        this.deprecatedFg = deprecatedFg;
    }

    public String getSiteLd() {
        return siteLd;
    }

    public void setSiteLd(String siteLd) {
        this.siteLd = siteLd;
    }

    public boolean isNawqaSiteFg() {
        return nawqaSiteFg;
    }

    public void setNawqaSiteFg(boolean nawqaSiteFg) {
        this.nawqaSiteFg = nawqaSiteFg;
    }

    public boolean isNasqanNmnSiteFg() {
        return nasqanNmnSiteFg;
    }

    public void setNasqanNmnSiteFg(boolean nasqanNmnSiteFg) {
        this.nasqanNmnSiteFg = nasqanNmnSiteFg;
    }
}